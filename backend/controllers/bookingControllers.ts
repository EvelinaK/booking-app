...
export const checkRoomBookingAvailability = catchAsyncErrors(
  async (req: NextRequest) => {
    ...
    //find bookings under this date range
...
export const getRoomBookedDates = catchAsyncErrors(async (req: NextRequest) => {
  ...
  //create new array based on range
  //type by-day
  // dates array in range
...
export const getBookingDetails = catchAsyncErrors(
  async (req: NextRequest, { params }: { params: { id: string } }) => {
    ...
    //is user ki booking nhi he ye so can't view it
...
const getLastSixMonthsSales = async () => {
  ...
  async function fetchSalesForMonth(
    ...
    //if found anything from above function -> res[0]
  ...
  //call above function fot last 6 month from curr date
  for (let i = 0; i < 6; i++) {
...
const getTopPerformingRooms = async (startDate: Date, endDate: Date) => {
  ...
  const topRooms = await Booking.aggregate([
    ...
    {
      $limit: 3, //only top 3 wale room lenge
    },

    //doing below because currenly have only room id but need to fetch that room name also
    //so deconstruct with document
